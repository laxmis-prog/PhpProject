...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4">Wellness Tracker</h2>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sleep_hours = filter_var($_POST['sleep_hours'], FILTER_VALIDATE_INT);
    $mood = htmlspecialchars(trim($_POST['mood']));
    $water_intake = filter_var($_POST['water_intake'], FILTER_VALIDATE_FLOAT);
    $exercise_type = htmlspecialchars(trim($_POST['exercise_type']));
    $exercise_duration = filter_var($_POST['exercise_duration'], FILTER_VALIDATE_INT);
    $nutrition_type = htmlspecialchars(trim($_POST['nutrition_type']));

    // Check that all required fields are filled
    if ($sleep_hours !== false && $water_intake !== false && $exercise_duration !== false &&
        !empty($mood) && !empty($exercise_type) && !empty($nutrition_type)) {
